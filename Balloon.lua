...
balloon.open = function(timed, mode)
    if timed then
        balloon.close_timer = mode and (mode == 15 or mode == 7) and 3 or balloon.settings.no_prompt_close_delay
        ui.timer_text:text(''..balloon.close_timer)
    else 
...
balloon.process_incoming_message = function(e)
	...
	if balloon.settings.display_mode >= 1 then
        ...
        result = balloon.process_balloon(e.message, mode)
    end
    return settings.DisplayMode == 1 and result or e.message_modified
end
...
balloon.process_balloon = function(message, mode)
    ...
    local timed = false
    if mode == 7 or mode == 15 then
        timed = true
    elseif not balloon.in_menu or not ends_with_prompt then
        timed = false
    end
    ...
    message = message:gsub(chat_color_codes.standard, "@R3S3T") --color code 1 (black/reset)
    message = message:gsub(chat_color_codes.item, "@ITEM") --color code 2 (green/regular items)
    message = message:gsub(chat_color_codes.key_item, "@KEYI") --color code 3 (blue/key items)
    message = message:gsub(chat_color_codes.blue, "@BLUE") --color code 4 (blue/???)
    message = message:gsub(chat_color_codes.magenta, "@MAGE") --color code 5 (magenta/equipment?)
    message = message:gsub(chat_color_codes.cyan, "@CYAN") --color code 6 (cyan/???)
    message = message:gsub(chat_color_codes.yellow, "@YELL") --color code 7 (yellow/???)
    message = message:gsub(chat_color_codes.orange, "@ORAN") --color code 8 (orange/RoE objectives?)
    ...
    message = message:gsub("@R3S3T", '\\cr')
    message = message:gsub("@ITEM", '\\cs('..ui._type.items..')')
    message = message:gsub("@KEYI", '\\cs('..ui._type.keyitems..')')
    message = message:gsub("@BLUE", '\\cs('..ui._type.keyitems..')')
    message = message:gsub("@MAGE", '\\cs('..ui._type.gear..')')
    message = message:gsub("@CYAN", '\\cs(0,159,173)')
    message = message:gsub("@YELL", '\\cs(156,149,19)')
    message = message:gsub("@ORAN", '\\cs('..ui._type.roe..')')
...
balloon.print_help = function(isError)
    ...
    local cmds = T{
        ...
        { '/balloon translate', 'Toggle balloon auto translation.' },
...
ashita.events.register('command', 'balloon_command_cb', function(e)
    ...
    if #args > 2 and args[2]:any('language') then
        if args[3] and language[string.lower(args[3])] then
            if balloon.settings.Language_name == language[string.lower(args[3])].name then
                print(chat.header(addon.name):append(chat.error("Language already set to %s"):format(balloon.settings.Language_name)))
                return
            else
                balloon.settings.lang = string.lower(args[3])
                balloon.settings.Language_name = language[string.lower(args[3])].name
                balloon.settings.Language_code = language[string.lower(args[3])].code
                print(chat.header(addon.name):append(chat.success("Language - %s"):format(balloon.settings.Language_name)))
                settings.save()
                return
            end
        else
            local str = ""
            local count = 0
            str = "Language "..args[3].." not supported - supported languages: \n"
